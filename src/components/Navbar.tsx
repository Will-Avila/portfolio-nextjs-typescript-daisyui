...
export default function Navbar() {
  ...
  return (
    <nav className="container-xl drawer font-semibold mx-auto">
      ...
      <div className="drawer-side z-50">
        ...
        <ul className="menu bg-base-200 min-h-full w-3/4">
          <li
            className="flex flex-row border-b mb-4 pb-4 items-center"
            onClick={closeDrawer}
          >
            <Logo />
            <span
              onClick={closeDrawer}
              className="absolute right-0  top-0 flex text-2xl"
            >
              x
            </span>
          </li>
          {menu.map((item) => (
            <li key={item.id}>
              <Link onClick={closeDrawer} href={item.url}>
                {item.name}
              </Link>
            </li>
          ))}
